...
def get_patch(img, gt, lr, scale, patch_size=16):
    ...
    return img[:,ty:ty + tp, tx:tx + tp, :], gt[:,ty:ty + tp, tx:tx + tp, :], lr[:,lr_ty:lr_ty + lr_tp, lr_tx:lr_tx + lr_tp,:]

class Tartanair_Dataset(Dataset):

    def __init__(self, root_path="/opt/data/private/dataset/VDSRDataset/TartanAir/", train=True,
                 txt_file='./tartanair_train.txt', scale=16,
                 transform=None):

        self.root_path = root_path
        self.temp_offset = 0
        self.rgb_prefix = 'image_left'
        self.rgb_suffix = '_left.png'
        self.d_prefix = 'depth_left'
        self.d_suffix = '_left_depth.npy'
        ...
        if self.train:
            for seq in seqlist:
                ...
                for idx in range(self.temp_offset+1, int(seq[2]) - self.temp_offset - 1):
                    gt_seqlist[-1].append(os.path.join(self.root_path, "HR_RGB", seq[0], seq[1], \
                                                       self.d_prefix, '{:06d}'.format(idx) + self.d_suffix))
                    lr_seqlist[-1].append(os.path.join(self.root_path, "LR", self.ss, seq[0], seq[1], '{:06d}'.format(idx) + self.d_suffix))
                    guide_seqlist[-1].append(os.path.join(self.root_path, "HR_RGB", seq[0], seq[1], \
    ...
    def load_and_stack(self, idx):
        ...
        if self.train:
            stacked_gt = stacked_gt / 30.0
            stacked_lr = stacked_lr / 30.0
