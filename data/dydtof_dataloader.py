...
class DyDToF_Dataset(Dataset):

    def __init__(self, root_path="/opt/data/private/dataset/VDSRDataset/DyDToF/", train=True,
                 txt_file='./dydtof_test.txt', scale=16,
                 transform=None):

        self.root_path = root_path
        self.temp_offset = 0
        self.rgb_suffix = '.jpeg'
        self.d_suffix = '.npy'
        ...
        if self.train:
            for seq in seqlist:
                ...
                for idx in range(self.temp_offset + 1, int(seq[2]) - self.temp_offset - 1):
                    gt_seqlist[-1].append(os.path.join(self.root_path, "DyDToF_HR_RGB", seq[0], seq[1], "depth", \
                                                       seq[3] + '{:04d}'.format(idx) + self.d_suffix))
                    lr_seqlist[-1].append(os.path.join(self.root_path, "DyDToF_LR", self.ss, seq[0], seq[1], "depth", \
                                                       seq[3] + '{:04d}'.format(idx) + self.d_suffix))
                    guide_seqlist[-1].append(os.path.join(self.root_path, "DyDToF_HR_RGB", seq[0], seq[1], "color", \
                                                          seq[3] + '{:04d}'.format(idx) + self.rgb_suffix))
            data_infos = {}
            data_infos['gt_path'] = \
    ...
    def load_and_stack(self, idx):
        ...
        if self.train:
            stacked_gt = stacked_gt / 40.0
            stacked_lr = stacked_lr / 40.0
